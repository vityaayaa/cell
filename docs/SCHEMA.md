...
Накопление под контролем с двух сторон:
- **Клиент:** `initialLoad` грузит только последние 500 записей (order by
  created_at desc, limit 500) и заменяет локальную копию целиком — телефон не
  тянет тысячи строк.
- **Сервер (миграция 014):** pg_cron-задача `purge_old_audit_log` каждый день в
  03:00 UTC удаляет записи старше 1 года. Таблица не растёт бесконечно.
