...
### Создано в I-04 (инвентарь)

**`src/features/shelf/useShelfData.ts`** — `export function useShelfData()` → `{ shelf, cells, products, materials }`. Читает через `useLiveQuery` из Dexie. `cells` — все ячейки shelf_id, `products` и `materials` — полные списки. Всегда возвращает массивы (не undefined).

**`src/features/shelf/ShelfGrid.tsx`** — `export function ShelfGrid(props: ShelfGridProps)`.
...
interface ShelfGridProps {
  ...
  products: Product[]
  materials: Material[]
  sessionId?: string           // для подсветки visitedCellIds
  visitedCellIds?: Set<string> // ячейки, в которые внесён остаток
  onLeafTap?: (cell: Cell) => void   // view-режим: тап на листовую ячейку
  onEditTap?: (cell: Cell) => void   // edit-режим: тап на любую ячейку
  onFlagTap?: (cell: Cell) => void   // тап на ⚠ флаг
}
```

Внутри: Level 1 — CSS Grid из корневых ячеек, drill-down через `useState<DrillEntry[]>`. В `ShelfPage` сейчас вызывается как `<ShelfGrid mode="view" ... onLeafTap={handleLeafTap} />` где `handleLeafTap` делает `navigate('/app/stock-entry/${cell.id}')`.

**`src/features/shelf/ShelfPage.tsx`** — `export default ShelfPage`. Монтирует Realtime-подписку на таблицу `cells`. Заглушка прогресс-бара: `<span>Обход: — из — ✓</span>`. **I-05 должна заменить заглушку на реальные данные**: передать `sessionId` и `visitedCellIds` из `db.stock_entries`. Progressbar ожидает кнопку "→ К заявке" которая вызывает `generateOrder()`.

**`src/features/shelf/CellCard.tsx`** — `export function CellCard(props: CellCardProps)`. Пропы включают `sessionId?: string` и `visitedCellIds?: Set<string>` для показа "✓ Внесено сегодня". Листовая ячейка показывает `lastEntryDate?: string | null`.

**`src/features/shelf/CellActionsSheet.tsx`** — edit-режим: split/merge/assign product. Не нужна в I-05.

**`src/features/shelf/cellUtils.ts`** — `isLeaf` берётся из `@/domain/bsp`: `isLeaf(nodeId: string, allNodes: BspNode[]): boolean`. Тип `Cell` совместим с `BspNode` (все нужные поля есть). Функция `getProductDisplayName(product)` — в `cellUtils.ts`.

**Realtime-подписка (паттерн, уже используется в ShelfPage и ShelfConfigPage):**
```typescript
useEffect(() => {
  const channel = subscribeToTable('cells', async (payload) => {
    if (payload.eventType === 'DELETE') await db.cells.delete(payload.old.id)
    else await db.cells.put(payload.new)
  })
...
- `BspNode.width_mm` и `height_mm` типизированы как `number | null | undefined` — Cell-массив совместим
- Stub StockEntryPage существует по маршруту `/app/stock-entry/:cellId` — I-05 заменяет его полной формой
