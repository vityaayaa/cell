...
- `src/data/store.ts` — `useAppStore()`: `activeSessionId`, `setActiveSession()`, `userRole`
...
- При нажатии: если внесено < 50% ячеек → Dialog "Внесено только N из M ячеек. Продолжить?" → при подтверждении `navigate('/app/order')`; если ≥ 50% → сразу navigate
...
export async function generateOrder(sessionId: string): Promise<string> {
  ...
  const lines = orderLines.map(l => ({
    ...
    product_name: /* S08: снимок имени */ await getProductDisplayName(l.product_id),
...
Создать `docs/sessions/prompts/I-06-checklist-sessions.md`.
