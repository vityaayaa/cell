...
interface ShelfGridProps {
  mode: 'edit' | 'view'
  // view mode: данные обхода для подсветки ячеек
  sessionId?: string
  visitedCellIds?: Set<string>
  // view mode: callback на тап листовой ячейки (если не передан — navigate внутри)
  onLeafTap?: (cell: Cell) => void
}
```

- `edit` → тап на ячейку открывает `CellActionsSheet` с операциями
- `view` → тап на листовую ячейку → вызывает `onLeafTap(cell)` если передан, иначе `navigate('/app/stock-entry/${cell.id}')`
- `sessionId` + `visitedCellIds` передаются в `CellCard` для отображения прогресса обхода

**Важно:** I-05 будет передавать `onLeafTap` и `visitedCellIds` в `ShelfPage`. Интерфейс должен это поддерживать.
