...
- **`OfflineIndicator.tsx` полностью реализован** (I-03): показывает offline · N записей, синхронизация..., снова онлайн. Перепроверить только визуальное качество.
- **`flushQueue()` и `checkOnline()` существуют** в `src/data/sync.ts` — написаны в I-03. В I-08 нужно только _вызывать_ их при reconnect (сейчас AppShell вызывает только `setOnline(true)`).
- **`isSyncing` и `syncQueueLength`** в store существуют и обновляются внутри `flushQueue()`.
- **`motion/react` (AnimatePresence)** — уже установлена и используется в `ChecklistPage.tsx` (fade+slide exit 150ms для completed строк). Повторно устанавливать не нужно.
- **`window.addEventListener('online'/'offline')`** — уже в AppShell, но без вызова `flushQueue()` и `initialLoad()`.
- **`pnpm build` — 0 ошибок** (верифицировано в I-07, коммит c843230).
